...
def More(contents, out=None, prompt=None):
  ...
  lines. A one character response is expected:

    <SPACE>, <RETURN>
      Print the next page of lines. Does not quit if there are no more lines.
    b, B
      Back up to the previous page of lines.
    j, J, +
      Move forward one line.
    k, K, -
      Back up one line.
    q, Q, ^C, ^D, <ESC>
      Quit paging and return to the caller.
    any other character
      Prompt again.
  ...
  if not out:
    ...
    out = sys.stdout
  attr = console_attr.GetConsoleAttr(out=out)
  width, height = attr.GetTermSize()

  # Construct a list of lines, splitting long lines into separate display lines.
  lines = []
  for line in contents.splitlines():
    lines += attr.SplitLine(line, width)
  if len(lines) <= height:
    out.write(contents)
    return
  ...
  if not prompt:
    prompt = '{bold}--More--({{percent}}%){normal}'.format(
        bold=attr.GetFontCode(bold=True), normal=attr.GetFontCode())
  clear = '\r{0}\r'.format(' ' * (attr.PrintWidth(prompt) + 2))
  ...
  while pos < len(lines):
    ...
    if nxt > len(lines):
      nxt = len(lines)
      pos = nxt - height
    out.write(os.linesep.join(lines[pos:nxt]) + os.linesep)

    # Handle the prompt response.
    percent = prompt.format(percent=100 * nxt / len(lines))
    while True:
      out.write(percent)
      c = attr.GetRawChar()
      out.write(clear)
